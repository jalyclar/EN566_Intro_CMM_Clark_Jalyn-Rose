...
class poisson:
    def __init__(self):
        ...
        self.Q=1
        self.grid_size=10 #setting grid size
    ...
    def gauss_seidel(self,DEBUG=False):
        #Variables 
        tolerance=10**-6
        #Gauss Seidel   
        if DEBUG==True:
        ...
        if DEBUG==True:
            print(f"Initial V(r) grid: {self.V}\n\n")
        iterations=int((self.grid_size**2)/2)
        for iter in range(iterations):
            max_change=0 #for convergence criteria 
            for i in range(1, self.grid_size-1): #rows
                for j in range(1, self.grid_size-1): #columns 
                    ...
                    if r<10: #boundary condition 
                        if self.rho[i,j]==0: #using laplace where there is no cahrge density 
                            V_n=1/4*(self.V[i+1,j]+self.V[i-1,j]+self.V[i,j+1]+self.V[i,j-1])
                        else: #using poisson where there is charge density 
                            V_n=1/4*(self.V[i+1,j]+self.V[i-1,j]+self.V[i,j+1]+self.V[i,j-1])+(self.rho[i,j]*self.h**2/self.e)
                        del_V=np.absolute(V_n-self.V[i,j]) #max chnage in V
                        self.V[i,j]=V_n
        ...
        if DEBUG==True:
            print(f"Final V(r) grid: {self.V}")
            
        #Equipotential Lines
        plt.contour(self.X,self.Y,self.V, levels=np.linspace(np.min(self.V),np.max(self.V),20),cmap='inferno')
        ...
        V_r = self.V[int(self.grid_size / 2), :]  # Potential along y=0 line
    ...
    #def partone(DEBUG=True):
        ...
        print(f'Converged after {N_iter} iterations.\n\n')
        plt.figure()
        plt.contour(X*h,Y*h,new_V, levels=np.linspace(-1,1,50),cmap='inferno') #RdYlBu
        ...
        surface=ax.plot_surface(X,Y,new_V,cmap='jet')
        ...
        r = np.linspace(-grid_size,grid_size,grid_size)*h #r distances along y=0 line 
    ...
    #def partthree(DEBUG=True):
        ...
        plt.figure()
        plt.plot(grids,iter_jacob,label='Jacobi Relaxation Method',color='pink')
        plt.plot(grids,iter_sor,label='Simultaneous Over-Relaxation Method',color='lightblue')
