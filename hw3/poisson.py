...
def partthree(DEBUG=True):
    grids=np.linspace(10,100,dtype=int) #grids=np.linspace(10,100,dtype=int) #For final analysis 
    iter_jacob=[]
    iter_sor=[]
    if DEBUG==True:
        print(f'grid sizes to use: {grids}')
    for grid_size in grids:
        _,_,_,N_iter=jacobi_relax(False,grid_size,10**-3)
        _,_,_,N_iter_s=sor(False,grid_size,10**-3)
